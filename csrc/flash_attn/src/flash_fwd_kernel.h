...
template<typename Kernel_traits, bool Is_dropout, bool Is_causal, bool Is_local, bool Is_alibi, bool Is_even_MN, bool Is_even_K, bool Return_softmax, typename Params>
inline __device__ void compute_attn_1rowblock(const Params &params, const int bidb, const int bidh, const int m_block) {
    ...
    for (int masking_step = 0; masking_step < n_masking_steps; ++masking_step, --n_block) {
        ...
        if (Is_alibi) {
            flash::apply_alibi(scores, n_block * kBlockN, m_block * kBlockM + (tidx / 32) * 16 + (tidx % 32) / 4,
                bidh, params.h, params.scale_softmax, params.alibi_slopes,
                binfo.actual_seqlen_k, binfo.actual_seqlen_q, kNWarps * 16);
        }
...
template<typename Kernel_traits, bool Is_causal, bool Is_local, bool Is_alibi, bool Is_even_MN, bool Is_even_K, bool Split, bool Append_KV, typename Params>
...
inline __device__ void compute_attn(const Params &params) {
    ...
    flash::compute_attn_1rowblock<Kernel_traits, Is_dropout, Is_causal, Is_local, Is_alibi, Is_even_MN, Is_even_K, Return_softmax>(params, bidb, bidh, m_block);
}
...
inline __device__ void compute_attn_splitkv(const Params &params) {
    ...
    flash::compute_attn_1rowblock_splitkv<Kernel_traits, Is_causal, Is_local, Is_alibi, Is_even_MN, Is_even_K, Split, Append_KV>(params, bidb, bidh, m_block, n_split_idx, num_n_splits);
}
